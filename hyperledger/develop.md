...
1. 创建系统链创世区块，也是首个配置区块，将来更新配置时追加新的配置区块即可
2. 创建通道的创世交易，并指定**channelID**，即**chainID**、**ledgerID**
3. 通过创世交易创建通道
4. 将peer加入指定channelID的通道
5. 安装链码，并指定**链码名和版本号**（同链码升级）
6. 根据链码名和版本号将链码实例化到指定channelID的通道
7. 使用channelID和链码名进行指定通道的链码交互
